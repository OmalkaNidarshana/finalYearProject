...
if( isset($_REQUEST['date_range']) ){
    ...
    foreach( $dateRangeArr as $dateCategory => $lbl){
        if( $dateCategory == 'today' ){
            $dateFltr = date("Y-m-d");
            $fltr= 'CREATED_DATE ='.getTextValue($dateFltr);
            ...
            $endDate = date("Y-m-d");
            $fltr = getTextValue($firstDay).'<= CREATED_DATE and CREATED_DATE<='.getTextValue($endDate);
            $order->setDateMode($dateCategory);
            $order->setDateLbl($lbl);
...
$page[] = $order->getSalesReportSummaryTable();
