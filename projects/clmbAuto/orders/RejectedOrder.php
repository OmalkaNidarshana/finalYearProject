...
class RejectedOrder{
    ...
    var $customerList;
    var $customerData;
    var $summaryFlds = array('ORDER_NUM','LINE_NUM','CUSTOMER_ID','BRAND','MODEL','BRISK','CATEGORY','ORDER_DATE',
                        'REJECTED_QTY','REJECTED_DATE','REJECTED_REASON');

    function RejectedOrder($link,$userInfo,$id=''){
        $this->link = $link;
        $this->id = $id;
        $this->userInfo = $userInfo;
        $this->dataTable = new SortableTable($this->link);
        $this->formatter = new RejectedOrderTableFormatter($this->link,$this->userInfo);
