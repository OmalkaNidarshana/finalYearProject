...
if( isset($_REQUEST['date_range']) ){
    ...
    foreach( $dateRangeArr as $dateCategory => $lbl){
        if( $dateCategory == 'today' ){
            $dateFltr = date("Y-m-d");
            $fltr= 'REJECTED_DATE ='.getTextValue($dateFltr);
            ...
            $endDate = date("Y-m-d");
            $fltr = getTextValue($firstDay).'<= REJECTED_DATE and REJECTED_DATE<='.getTextValue($endDate);
            $order->setDateMode($dateCategory);
            $order->setDateLbl($lbl);
...
$page[] = $order->getRejectOrderReportSummaryTable();
