<?php
    class ReOrderTableFormatter{
        
        var $link;
        var $userInfo;
        var $categoryIds = array();

        function ReOrderTableFormatter($link,$userInfo){
            $this->link = $link;
        ...
        function formatters($id,$value,$data){
             ...
             switch($id){
                case 'ORDER_NUM':
                    $formatter = '<a href="'.makeLocalUrl('orders/order_details.php','sec=ORDER&id='.$data['REF_ORDER_ID']).'">'.$value.'</a>';
                break;
                case 'CUSTOMER_ID':
